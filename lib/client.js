      ...
      function PianoNavOverlayInner(props) {
        ...
        try {
          if (currentId && ctxRef) {
            ...
            if (convBinding && typeof convBinding.target === 'function' && convBinding.views && typeof convBinding.views.get === 'function') {
              const chatView = convBinding.views.get('chat')
              snapDiag.chatViewFound = !!chatView
              if (chatView) {
                snapDiag.chatViewKeys = Object.keys(chatView).slice(0, 15)
                snapDiag.chatViewSnapType = chatView.snapshot ? typeof chatView.snapshot : 'none'
              }
              if (chatView && chatView.snapshot && typeof chatView.snapshot === 'object') {
                snapDiag.snapKeys = Object.keys(chatView.snapshot).slice(0, 20)
                snapDiag.snapHasNav = !!(chatView.snapshot.navigation && typeof chatView.snapshot.navigation.items === 'function')
                const chatTarget = convBinding.target('chat')
                snapDiag.hasTargetSub = !!(chatTarget && typeof chatTarget.subscribe === 'function')
                if (chatTarget && typeof chatTarget.subscribe === 'function') {
                  subscribeSnap = chatTarget.subscribe.bind(chatTarget)
                  getSnap = () => chatView.snapshot
                }
              }
            }
