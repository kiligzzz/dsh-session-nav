      ...
      function PianoNavOverlayInner(props) {
        ...
        try {
          if (currentId && ctxRef) {
            ...
            } catch (err) { convBinding = null }
            if (convBinding && typeof convBinding.target === 'function' && convBinding.views && typeof convBinding.views.get === 'function') {
              // target('chat').getSnapshot() 返回 view 条目（含 .snapshot）；
              // 真正的 chat snapshot 是 view.snapshot（builder 产物，含
              // order/nodes/navigation）。官方 useChat 订阅的就是它。
              // 订阅复用 BoundConversation 的 snapshot store 通知。
