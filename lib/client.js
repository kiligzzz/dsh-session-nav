  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      const buildEntries = (snapshot) => {
        ...
        // 0.1.2 兼容兜底：chat.order 只含 visibility=visible 的节点，
        // 部分 assistant-step（hidden）不进 order → modelText 缺失。
        // 官方 TurnNavigator 用 snapshot.locations.getTurn()（含 hidden 的完整
        // turn 索引）取回复文本，这里做同样兜底。
        if (snapshot.locations && typeof snapshot.locations.getTurn === 'function') {
          for (const entry of entries) {
            if (entry.modelText) continue
            // 该 user entry 对应的 turn：从 byTurn 反查
            let turn = null
            for (const [t, e] of byTurn) { if (e === entry) { turn = t; break } }
            if (turn === null) continue
            const keys = snapshot.locations.getTurn(turn) || []
            for (const k of keys) {
              const n = (chat && chat.nodes && typeof chat.nodes.get === 'function') ? chat.nodes.get(k) : null
              if (!n) continue
              if (n.kind === 'assistant-step' || n.kind === 'assistant') {
                const data = n.data && typeof n.data === 'object' ? n.data : n
                appendModelText(entry, data.blocks)
                if (entry.modelText) break
              }
            }
          }
        }
