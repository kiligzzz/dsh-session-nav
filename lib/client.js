    ...
    factory: (require) => {
      ...
      function PianoNavOverlayInner(props) {
        ...
        const compute = () => {
          ...
          if (!sp || list.length === 0 || isScrollportOccluded(sp) || hasModalOverlapping(sp)) {
        ...
        // conversation.view 切 Tab 时 chat 容器的可见性（display:none 翻转检测）
        const scrollportVisibleRef = useRef(
          typeof document !== 'undefined' && !!document.querySelector('[data-conversation-scroll]')
            ? document.querySelector('[data-conversation-scroll]').getClientRects().length > 0
            : false,
        )
        const attachShellObserver = () => {
          ...
          if (!bodyMoRef.current && typeof MutationObserver !== 'undefined') {
            bodyMoRef.current = new MutationObserver(() => {
              ...
              // conversation.view Tab 切换：官方切到轨迹/Agent调度/记忆系统时
              // 不卸载 chat 容器，而是 style.display='none' 隐藏（DOM 保留）。
              // findScrollport() 对 display:none 返回 null，compute() 的
              // !sp→hidden 分支能正确隐藏钢琴键——缺的只是触发信号。
              // 与 modal 同模式：监听 scrollport 的「可见→不可见」翻转，
              // 只在翻转时 schedule，避免 token 流式渲染每帧重算。
              const sp = document.querySelector('[data-conversation-scroll]')
              const spVisible = !!sp && typeof sp.getClientRects === 'function' && sp.getClientRects().length > 0
              if (spVisible !== scrollportVisibleRef.current) {
                scrollportVisibleRef.current = spVisible
                schedule()
              }
            })
            bodyMoRef.current.observe(document.body, { childList: true, subtree: true })
          }
        }
        useEffect(() => () => {
