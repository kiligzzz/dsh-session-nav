  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      function PianoNavOverlayInner(props) {
        ...
        // shell.overlay 是 frame-wide 浮动层——切到非 chat 视图时 overlay 仍在，
        // 必须按「会话流容器是否真实可见」决定显示，否则其他 Tab 上会残留钢琴键。
        // 注意：官方切 view 可能保留 DOM（display:none 隐藏 chat 容器），
        // 所以不能只查存在性，要查可见性（有尺寸 + offsetParent 非 null）。
        const isChatVisible = () => {
          if (typeof document === 'undefined') return true
          const sp = document.querySelector('[data-conversation-scroll]')
          if (!sp) return false
          // 可见性三重校验：布局尺寸、offsetParent（display:none 时 null）、
          // 且至少一个祖先不是 visibility:hidden
        ...
        const [chatVisible, setChatVisible] = useState(isChatVisible)
        useEffect(() => {
          ...
          mo.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style', 'hidden', 'data-state'] })
        ...
        // 非「对话」视图（轨迹/Agent 调度/记忆系统等 Tab）不渲染钢琴键
